...
sealed class Loader(val id: String) {
	...
	object Fabric : Loader("fabric") {
		...
		override fun generateManifest(ctx: Context): String {
			val basePackage = "${ctx.modGroup}.${ctx.modId}"
			val fabricEnv = when (ctx.environment.lowercase()) {
				"client" -> "client"
				"server" -> "server"
				else -> "*"
			}
			val manifest = FabricManifest(
				...
				environment = fabricEnv,
				accessWidener = null,
				entrypoints = mapOf(
					"main" to listOf("$basePackage.platform.fabric.FabricEntrypoint")
				),
